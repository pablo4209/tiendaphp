	...
	$(function(){		
		...
		{
			var opciones="toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, width=650, height=365, top=85, left=140";
			window.open($('#pathjs').val()+'upimg/cargar_imagen.php?d=tmp&path='+ $('#pathjs').val() +'&ctrl=Imagen',"",opciones);
			return false;
		}
		...
		$('#clrImagen').click(function(){
			if($('#Imagen').val() != "")
			{
				...
				{					
					$('#Imagen').val("");						
					$("#imgImagen").attr("src",$('#pathimg').val()+'NoDisponible.jpg');
				}
			}
