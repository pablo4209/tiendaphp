...
class Reparaciones extends Conectar 
{
    private $u;
    private $campos = " `id` ,  `idEquipo` ,  `idCliente` ,  `idLista` ,  `Razonsocial` ,  `Nombre` ,  `Dom` ,  `Tel` ,  `Tel2` ,  `Email` ,  `Horariocontacto` ,  `idEstado` ,  DATE_FORMAT(Fecha,'%d/%m/%Y') as 'Fecha' , DATE_FORMAT(Fechaingreso,'%d/%m/%Y') as `Fechaingreso` , DATE_FORMAT(Fechaentrega,'%d/%m/%Y') as `Fechaentrega` ,  `Falladeclarada` ,  `Observaciones` ,  `Observacionespriv` ,  `Accesorios` " ;
    private $tabla = " `tbrep` ";
	private $orden = " order by Nombre asc";
    private $countAjax =0;
    ...
    {
        $sql="SELECT  ".$campos." FROM ".$tabla.$orden;
    ...
    public function add()
    {
        ...
        if(!parent::getRowId("SELECT tbpro.Codigo FROM tbpro WHERE Codigo=?",$_POST["Codigo"])) //si es codigo ya existe no lo crea
		{
