...
if(isset($_POST["grabar"]) && $_POST["grabar"] !="" )
{
	...
	//////////////////////////tratar la IMAGEN
	if(isset($_POST["Imagen"]) && $_POST["Imagen"]!="" )  // ATENCION! si la imagen no se cambia
	{																											// este campo no esta seteado
					write_log("pro-edit:" , "post imagen");
	...
	{
		write_log("pro-edit:" , "prod editado correctamente");
		/////////////////////MODIFICAR LISTAS en tbpro_precios		*** REVISADO	OK
		if(isset($_POST["margen"]))
		{
			write_log("pro-edit:" , "array margen encontrado, elementos: ".sizeof($_POST["margen"]) );
			$pp=new proPrecios();
		...
		{
			write_log("pro-edit:" , "array stock encontrado, tamaño: ".sizeof($_POST["stock"]));
			$ps = new proStock();
...
{		//VISUALIZACION DE PRODUCTO
	...
	if(isset($_GET["id"]) && $_GET["id"]!="")
	{
		...
		if(!empty($p))
		{
			...
			$selCat2 = $cat->getSelCategorias(0,0 ,"Seleccionar Categoria",0 , "idCategoria2", false); //sin seleccionar ninguno
			...
			if(is_file(PATH_IMG.$imgdir.$p[0]["Imagen"])){
							...
							$imgpath = PATH_IMG."255x320.png"; //ruta por defecto cuando la imagen no se encuentra
			}
